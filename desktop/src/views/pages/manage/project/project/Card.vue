<template>
  ...
  >
    ...
    <layer v-if="showShareFrom" style="width: 100%" @closed="toggleShare">
      <b-card style="left:330px; top: -7px; width: 300px">
        <share-link
          :project="item"
          :stage="stage"
          @createdLink="createdLink($event)"
        ></share-link>
      </b-card>
    </layer>
    ...
    <div class="card-body py-2">
      ...
      <div class="details">
        ...
        <div
          v-if="stage.status !== 'FINISHED'"
          class="mt-3"
        >
          <loading-button
            size="lg"
            block
            :disabled="detailsForm.busy"
            @click="toggleShare"
            >{{ $t("Share Task") }}
          </loading-button>
          <div class="mt-3">
            <share-detail 
              :shareLinks="shareLinks.filter((o) => o.projectId == item.id)"
              @delLink="deleteLink($event)"
            >
            </share-detail>
          </div>
        </div>
...
export default {
  ...
  methods: {
    async deleteLink(id){
			await this.$store.dispatch("shareLink/delete", new GQLForm({
				id: id
			}));
    },
    async createdLink(data){
      this.$emit("done");
    },
    ...
    initEdit() {
      ...
      this.editItem = {
        ...
        shareAccessRoles: this.item.shareAccessRoles,
      };
    },
    async toggleEdit() {
      if (!this.editing) {
    ...
    async loadDetails() {
      ...
      await this.$store.dispatch('shareLink/findAll', {force: true});
    },
    async nextStage() {
    ...
    async toggleShare() {
      this.showShareFrom = !this.showShareFrom;
    },
  },
};
</script>
