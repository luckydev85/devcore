 <template>
  <div class="page" style="width: 300px; min-height: 50px">
    ...
    <restrict-role
      name="share"
      v-model="getShareAccessRoles"
      :labelText="$t('Share Access Restriction')"
      :infoText="$t('Share Access Info')"
      :type="type"
      @opened="setType"
      @changed="saveSelection"
      :items="getAllRoles"
...
export default {
  ...
  computed: {
    ...
    getShareAccessRoles: {
      get() {
        return this.companyRoles.filter(
          (cr) => this.advancedForm.shareAccessRoles.indexOf(cr.id) > -1
        );
      },
      set(value) {
        this.advancedForm.shareAccessRoles = value;
      },
    },
  },
  props: {
  ...
  mounted() {
    ...
    if (this.form.shareAccessRoles) {
      this.advancedForm.shareAccessRoles = this.form.shareAccessRoles ?? [];
    }
    if (this.value && this.value.shareAccessRoles) {
      this.advancedForm.shareAccessRoles =
        this.value.shareAccessRoles ?? [];
    }
  },
  methods: {
    setType(_type) {
