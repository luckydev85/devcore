        ...
        <div class="items">
          <div class="d-flex overflow-hidden justify-content-end">
            <div class="flex-grow-1" style="max-width: 65%">
              <slot
                name="header-display"
                v-bind="{ values: dataValue || [], items: items }"
              ></slot>
            </div>
            <div
...
export default {
    ...
    if (!this.value) {
      ...
      this.dataValue = this.value.filter((v) => {
        if (typeof v === "object" && v != null)
          return this.items.find((o) => this.getItemValue(o) === v[this.valueKey]) != null;
        else
          return this.items.find((o) => this.getItemValue(o) === v) != null;
      });

      this.dataValue = this.dataValue.map(function(item) {
  ...
  methods: {
    ...
    isSelected(item) {
      ...
      return this.dataValue.some((i) => this.getItemValue(i) === itemValue);
    },
    isItemDisabled(item) {
    ...
    toggleItem(item, event) {
      ...
      if (itemFound == false) {
        this.filteredItems.push(item[this.valueKey]);
      }
